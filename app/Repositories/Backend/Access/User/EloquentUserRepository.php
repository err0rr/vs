    ...
    public function getForDataTable($status = 1, $trashed = false)
    {
        ...
        if ($trashed == "true") {
            return User::onlyTrashed()
                ->select(['id', 'name', 'email', 'status', 'confirmed', 'created_at', 'updated_at', 'deleted_at','user_type'])->where('id',"!=", 1)
                ->get();
        }

        return User::select(['id', 'name', 'email', 'status', 'confirmed', 'created_at', 'updated_at', 'deleted_at','user_type'])
            ->where('status', $status)->where('id',"!=", 1)
            ->get();
    }
