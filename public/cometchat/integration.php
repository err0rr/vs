...
define('DB_SERVER',			'localhost' /* $db['connections']['mysql']['host'] */				); // Database host
define('DB_PORT',			"3306"											); // Database port
define('DB_USERNAME',		'root' /* $db['connections']['mysql']['username'] */			); // Database username
define('DB_PASSWORD',		'' /* $db['connections']['mysql']['password'] */			); // Database password
define('DB_NAME',			'bemygirl' /* $db['connections']['mysql']['database'] */			); // Database name
...
$db_usertable_name = 'name'; //'name';					// Name containing field in the users or members table
$db_avatartable = ' ';
...
class Integration{
	...
	function getAvatar($image) {
		$sql = "select photo from users where id=".$image;
		$result = sql_query($sql, array(), 1);
		if(count($result) > 0){	
			$row = sql_fetch_assoc($result);		
			$profile_photo = $row['photo'];
			if(file_exists('../img/users/'.$profile_photo)){
				return '../img/users/'.$profile_photo;
			}else{
