	...
	private function prepareFiles(){
        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                $metas['extension'] = $field['Field_Type'] == 'link' && empty($metas['extension']) ? "jpg" : $metas['extension'];
                $metas['old_name'] = $field['name'][$i];
                $metas['size'] = $field['size'][$i];
                $metas['size2'] = $this->formatSize($metas['size']);
                $metas['name'] = $this->generateFileName($this->options['title'], array('name'=>substr(rand(1111,9999)."_".time(), 0, (!empty($metas['extension']) ? -(strlen($metas['extension'])+1) : strlen($metas['old_name']))), 'size'=>$metas['size'], 'extension'=> $metas['extension']));
