<?php /*Hello dear member<br>
Name :     {{$nameuser}}<br>
Book Date: {{$date}}<br>
Book Time: {{$starttime}}-{{$endtime}}<br>
Price:     {{$rate}}<br>
Title:     {{$titlebookescort}} */?>
...
<body style="background: #f9f9f9;font-family: 'CenturyGothic';">
    ...
    <thead>
      <tr>
            <td colspan="2"><img src="img/email/images/logo.png"></td>
        </tr>
    </thead>
        <tbody style="background: #fff;">
        
        <tr>  
            <td colspan="2"><img src="img/email/images/logo.pngimages/ban2.png" style="width: 100%"></td>
        </tr>
        <tr>  
            <td colspan="2"><h4 style="text-align: center;margin:20px;font-size: 45px;">Booking Request</h4></td>
        </tr>
        <tr>  
            <td colspan="2"><p style="margin:10px 0px;padding: 0px 10px">Hi Dear Member</p></td>
        </tr>
        <tr>  
            <td style="padding: 0px 10px">Name</td><td style="padding: 0px 10px">: {{ ucfirst($nameuser) }}</td>
        </tr>
        <tr>  
            <td style="padding: 0px 10px">Book Date</td><td style="padding: 0px 10px">: {{ date('d-m-Y', strtotime($date)) }}</td>
        </tr>
        <tr>  
            <td style="padding: 0px 10px">Book Time</td><td style="padding: 0px 10px">: {{ date('h.i A', strtotime($starttime)) }} - {{ date('h.i A', strtotime($endtime)) }}</td>
        </tr>
        <tr>  
            <td style="padding: 0px 10px">Price</td><td style="padding: 0px 10px">: {{ $rate }}</td>
        </tr>
        <tr>  
            <td style="padding: 0px 10px">Title</td><td style="padding: 0px 10px">: {{ ucfirst($titlebookescort) }}</td>
        </tr>
        <tr style="text-align: center;">  
            <td colspan="2" style="padding: 30px 5px 20px;"><a href="#" style="text-align: center;color: #fff;background: #ffb200;padding: 10px 20px;border-radius: 5px;text-decoration: none;">Confirm Now</a></td>
        </tr>
        </tbody>
    </table>
    <table style="width: 800px;border: none;color: #333;margin:30px auto;font-size: 25px;text-align: center;">
      <tbody>
          ...
          <tr>
            <td colspan="2" style="padding-top: 15px;"><img src="img/email/images/logo.png"></td>
          </tr>
      </tbody>
